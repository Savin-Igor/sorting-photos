...
#### ⚠️ Важно: Исходные файлы НЕ удаляются

**Приложение НИКОГДА не удаляет файлы из source директории:**
- ✅ Только создает копии в destination директории
- ✅ Исходные файлы остаются на месте
- ✅ Безопасно для работы с ценными данными
...
**Пример:**
```
photo.jpg          # Первая копия
photo-a1b2c3d4.jpg # Коллизия (добавлен префикс хеша)
```

**Исходный файл остается в source директории без изменений.**
...
**A:** Да! Приложение:
- ✅ **НИКОГДА не удаляет исходные файлы** - они остаются в source директории
- ✅ Только создает копии в destination директории
