...
stateDiagram-v2
    ...
    Pending --> Uploading: Начата загрузка
    Pending --> NotFound: Файл не существует
    Pending --> Archived: Исключен из обработки
    
    Uploading --> Uploaded: Файл загружен
    Uploading --> Failed: Ошибка загрузки
    Uploading --> Paused: Приостановлено (квоты)
    
    Uploaded --> InBatch: Добавлен в батч
    InBatch --> Completed: Медиа-элемент создан
    InBatch --> Failed: Ошибка создания
    
    Failed --> Retrying: Повторная попытка
    Retrying --> Uploading: Повтор загрузки
    Retrying --> Failed: Превышено кол-во попыток
    
    Paused --> Uploading: Возобновление
...
- **`PENDING`** - Задача создана, файл готов к обработке
- **`UPLOADING`** - Файл загружается на сервер Google
- **`UPLOADED`** - Файл загружен, получен uploadToken (еще не виден в Google Photos)
- **`IN_BATCH`** - Файл добавлен в батч для создания медиа-элемента
- **`COMPLETED`** - Медиа-элемент создан, файл виден в Google Photos
- **`FAILED`** - Ошибка при загрузке или создании медиа-элемента
- **`PAUSED`** - Загрузка приостановлена (например, из-за квот API)
...
- 🎥 **Большие видео** (для уменьшения размера)

**Не сжимать:**
- ❌ Уже оптимизированные файлы
- ❌ Файлы меньше минимального размера (настраивается)
- ❌ Когда важно сохранить качество
- ❌ RAW для профессиональной обработки

### ✅ Сохранение метаданных

При сжатии **полностью сохраняются**:
- ✅ **EXIF данные** (для изображений) - копируются через exiftool
- ✅ **Дата создания** (для видео) - устанавливается через ffmpeg
- ✅ **Метаданные** - все важные теги сохраняются
